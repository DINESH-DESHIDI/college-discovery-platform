...
try {
  ...
  assert(safe(undefined, "fallback") === "fallback", "safe() resolves fallback if value is undefined");
} catch (e) {
...
try {
  ...
  assert(cn("p-4 bg-red-500", "p-6") === "bg-red-500 p-6", "cn merges tailwind padding override correctly");
} catch (e) {
...
try {
  ...
  const filterColleges = (filters) => {
    ...
    return colleges.filter((c) => {
      ...
      if (filters.category !== "All" && !c.courses.some((co) => co.category === filters.category)) return false;
      return true;
    });
  };

  const mumbaiMatches = filterColleges({ q: "Mumbai", location: "All", type: "All", minRating: 0, maxFees: 3000000, category: "All" });
  assert(mumbaiMatches.length === 1 && mumbaiMatches[0].id === "iit-bombay", "Filter matches Mumbai to IIT Bombay");

  const highRatedGov = filterColleges({ q: "", location: "All", type: "Government", minRating: 4.8, maxFees: 3000000, category: "All" });
  ...
  const lowFeesOnly = filterColleges({ q: "", location: "All", type: "All", minRating: 0, maxFees: 100000, category: "All" });
  // DU (30k) and AIIMS (6k)
